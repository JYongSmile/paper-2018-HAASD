...
for i in range(10,200,10):
    clf_gb = GradientBoostingClassifier(n_estimators=i)
    clf_gb.fit(batch_x,batch_y)

    y_pred_rf = clf_gb.predict(test_x)
    acc_rf = accuracy_score(test_y,y_pred_rf)
    print("%s n_estimators = %d, GradientBoostingClassifier accuracy:%f" %(datetime.now(),i,acc_rf))
...
y_scores = cross_val_predict(clf_gb, test_x, test_y, cv=3, method="decision_function")
